...
App.renderClients = async function (host) {
  ...
  const applyFilter = () => {
    ...
    state.list = state.all.filter((row) => {
      const s = `${row.company_name || ""} ${row.full_name || ""} ${row.phone1 || ""} ${row.phone2 || ""}`.toLowerCase();
      return s.includes(q);
    });
  };
  ...
  const renderList = () => {
    ...
    if (!state.list.length) {
      listHost.appendChild(el("div", { class: "muted2" }, t("empty") || "Empty"));
      return;
    }

    const list = el("div", { class: "cliList" }, state.list.map(row => {
      ...
      const subParts = [
        ...
        row.phone1 || "",
        row.phone2 || "",
        dictLabel(state.refs.cities, row.city_id),
        dictLabel(state.refs.sources, row.source_id),
        dictLabel(state.refs.spheres, row.sphere_id),
      ].filter(Boolean);
      ...
      return el("div", { class: "cliRow" },
        ...
        el("div", { class: "cliActions" },
          el("button", { class: "iconBtn", type: "button", onClick: () => openClientModal(row.id) }, t("open") || "Open")
        )
      );
    }));
  ...
  const loadList = async () => {
    if (listHost) {
      listHost.innerHTML = "";
      listHost.appendChild(el("div", { class: "muted2" }, t("loading") || "Loading…"));
    }
    const r = await API.clients.list(state.tab, ""); // грузим всё
    state.all = r.data || [];
    applyFilter();
    renderList();
  };

  const openClientModal = async (id) => {
    try {
      const r = await API.clients.get(id);
      const row = r.data;
      // тут оставляю твою текущую логику модалки (не ломаю)
      // если хочешь — потом улучшим UI модалки отдельно
      Modal.open(`#${row.id}`, el("div", { class: "vcol gap8" }, JSON.stringify(row, null, 2)), [
  ...
  const renderShell = () => {
    ...
    const tabs = el("div", { class: "tabs" },
      el("button", {
        class: `tabBtn ${state.tab === "company" ? "active" : ""}`,
        type: "button",
        onClick: async () => { state.tab = "company"; await loadList(); }
      }, t("clients_companies")),
      el("button", {
        class: `tabBtn ${state.tab === "lead" ? "active" : ""}`,
        type: "button",
        onClick: async () => { state.tab = "lead"; await loadList(); }
      }, t("clients_leads")),
    );

    qInp = el("input", { class: "input", value: state.q, placeholder: t("clients_search") });
    qInp.addEventListener("input", () => {
      state.q = qInp.value || "";
      applyFilter();
      renderList();
    ...
    const createBtn = (state.tab === "company")
      ? el("button", { class: "btn", type: "button", disabled: !canCreateCompany }, t("clients_create_company"))
      : el("button", { class: "btn", type: "button", disabled: !canCreateLead }, t("clients_create_lead"));

    const top = el("div", { class: "card cardPad vcol gap10" },
      el("div", { class: "cliTop" },
        tabs,
        el("div", { class: "hrow gap10", style: "flex-wrap:wrap" }, qInp, createBtn)
      )
    );

    listHost = el("div", { class: "vcol gap10", style: "margin-top:12px" });
