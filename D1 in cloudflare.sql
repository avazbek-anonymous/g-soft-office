...
CREATE TABLE dict_cash_income_categories (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name_ru TEXT NOT NULL,
  name_uz TEXT NOT NULL,
  name_en TEXT NOT NULL,
...
CREATE TABLE dict_cash_expense_categories (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name_ru TEXT NOT NULL,
  name_uz TEXT NOT NULL,
  name_en TEXT NOT NULL,
...
CREATE TABLE dict_payment_methods (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name_ru TEXT NOT NULL,
  name_uz TEXT NOT NULL,
  name_en TEXT NOT NULL,
  currency_mode TEXT NOT NULL DEFAULT 'ANY' CHECK (currency_mode IN ('ANY','USD','UZS')),
  sort INTEGER NOT NULL DEFAULT 1000,
  is_active INTEGER NOT NULL DEFAULT 1,
  created_at INTEGER NOT NULL DEFAULT (CAST(strftime('%s','now') AS INTEGER)),
  updated_at INTEGER NOT NULL DEFAULT (CAST(strftime('%s','now') AS INTEGER))
...
CREATE TABLE cash_transactions (
  ...
  object_type TEXT CHECK (object_type IS NULL OR object_type IN ('course','project')),
  object_id INTEGER,
...
CREATE INDEX idx_cash_transactions_created_at
ON cash_transactions(created_at DESC, id DESC);

CREATE INDEX idx_cash_transactions_status
ON cash_transactions(status);

CREATE INDEX idx_cash_transactions_movement
ON cash_transactions(movement_type);

CREATE INDEX idx_cash_transactions_counterparty
ON cash_transactions(counterparty_client_id);

CREATE INDEX idx_cash_transactions_object
ON cash_transactions(object_type, object_id);

CREATE INDEX idx_cash_transactions_author
ON cash_transactions(created_by);

CREATE TRIGGER trg_dict_cash_income_categories_updated_at
AFTER UPDATE ON dict_cash_income_categories
WHEN NEW.updated_at = OLD.updated_at
BEGIN
  UPDATE dict_cash_income_categories SET updated_at = (CAST(strftime('%s','now') AS INTEGER)) WHERE id = NEW.id;
END;

CREATE TRIGGER trg_dict_cash_expense_categories_updated_at
AFTER UPDATE ON dict_cash_expense_categories
WHEN NEW.updated_at = OLD.updated_at
BEGIN
  UPDATE dict_cash_expense_categories SET updated_at = (CAST(strftime('%s','now') AS INTEGER)) WHERE id = NEW.id;
END;

CREATE TRIGGER trg_dict_payment_methods_updated_at
AFTER UPDATE ON dict_payment_methods
WHEN NEW.updated_at = OLD.updated_at
BEGIN
  UPDATE dict_payment_methods SET updated_at = (CAST(strftime('%s','now') AS INTEGER)) WHERE id = NEW.id;
END;
...
WHEN NEW.updated_at = OLD.updated_at
BEGIN
  UPDATE cash_transactions SET updated_at = (CAST(strftime('%s','now') AS INTEGER)) WHERE id = NEW.id;
END;
